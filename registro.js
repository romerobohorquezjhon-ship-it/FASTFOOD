...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Si estamos en la página de mesero, mostrar nombre y menú
...
function renderizarMenu() {
    ...
    contenedor.innerHTML = productos.map(cat => `
        <div class="mb-4">
            <h3 class="text-cyan-500 text-[10px] font-black mb-3">${cat.categoria}</h3>
            <div class="grid grid-cols-2 gap-2">
                ${cat.items.map(p => `
                    <button onclick="agregarItem('${p.n}', ${p.p})" class="bg-gray-800 p-3 rounded-lg border border-gray-700 text-white text-xs">
                        ${p.n} <br> <span class="text-cyan-400">$${p.p}</span>
                    </button>
                `).join('')}
            </div>
        </div>
...
// ==========================================
// 3. LÓGICA DE COCINA (VISUALIZACIÓN)
// ==========================================
function cargarPedidos() {
    const contenedor = document.getElementById('listaPedidos');
    if (!contenedor) return; // Si no estamos en la página de cocina, no hacer nada
...
// Iniciar el monitor si existe el contenedor de cocina
